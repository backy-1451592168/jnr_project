...
var util = require("../../utils/colorPicker.js");
...
Component({
  ...
  methods: {
    _selectColor: function(e) {
      // 计算 元素距离页面顶部和左边的距离
      this.getPickerTopOffset(({top, left}) => {
        // console.log(e);
        var x = ((e.detail.x - left) * (100 / this.data.size)).toFixed(0) - 50;
        var y = -(((e.detail.y - top) * (100 / this.data.size)).toFixed(0) - 50);
        ...
        this.triggerEvent('selectColor', {
          ...
          color: util.hsv2rgb(h, s, this.data.brightness)
        })
      });
    ...
    // 异步操作 获取元素位置
    getPickerTopOffset(callback) {
      const query = wx.createSelectorQuery().in(this);
      query.select('#imgPicker').boundingClientRect((rect) => {
        if (rect) {
          callback({top: rect.top, left: rect.left});
        } else {
          console.error('选择器没有匹配到任何元素');
        }
      }).exec();
    }
  },
